...
- (instancetype)init {
    ...
    if (self) {
        ...
        UIViewController *shopVC = [shopService nativeFetchDetailViewController:@{@"key":@"shop Module"}];

        if ([shopVC isKindOfClass:[UIViewController class]]) {
            [self registerViewController:shopVC title:@"shop Module" iconName:nil];
        }
        
        id<UserTrackServiceProtocol> userTrackService = [[BeeHive shareInstance] createService:@protocol(UserTrackServiceProtocol)];
        UIViewController*userTrackVC= [userTrackService fetchUserTrackViewController:@{@"key":@"UserTrack"}];
        if ([userTrackVC isKindOfClass:[UIViewController class]]) {
            [self registerViewController:(UIViewController *)userTrackVC title:@"userTrackVC" iconName:nil];
        }
